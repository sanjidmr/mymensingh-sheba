...
export default async function KajerBuaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const profile = await fetchStaffProfileById(id);
  if (!profile || profile.serviceSlug !== 'kajer-bua') notFound();
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="relative bg-gradient-to-br from-emerald-700 via-emerald-800 to-emerald-900 text-white overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="flex items-center gap-2 text-sm text-emerald-100 mb-3">
          ...
          <p className="text-emerald-100 mt-2 text-base sm:text-lg max-w-2xl">{profile.titleBn}</p>
        </div>
      </section>
