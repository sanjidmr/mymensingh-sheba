...
- Brand: Noto Sans Bengali via `next/font/google`. Exact palette in `app/globals.css` `@theme`:
  - Deep Forest Green `--color-brand-700` `#123C32`, Secondary Green `--color-brand-600` `#176B57`, Lime Accent `--color-accent-400` `#B8E94E`, Warm Ivory `--color-mist-50` `#F7F7F2`, Main Text `--color-ink-900` `#17201D`, Muted `--color-ink-500` `#66736E`, Border `--color-brand-100` `#E3E7E2`.
  - `--color-emerald-*` is aliased to the same brand scale so legacy pages (login, admin, tolet, services…) share the green identity without per-class migration.
  - Body `bg-mist-50`; `::selection` lime; desktop `viewport.themeColor` `#F7F7F2`.
- Design rules: no gradients/glassmorphism/floating blobs/AI-template look; lime used only for small highlights + CTAs; rounded-2xl max (pills full-round); subtle shadows; `backdrop-blur` only on the functional navbar/bottom-nav surfaces.
- Hero — always a hard-separated section below the navbar (`HeroCarousel.tsx` + `HeroSection.tsx`, one state machine for all breakpoints, slides `/sheba1.png`…`/sheba4.png` ≈ 16:9, `.png`→`.jpg` probing then a flat `brand-800` fallback panel):
  - Desktop (lg+): the carousel images are FULL-WIDTH and IMAGE-ONLY (cinematic `h-[480px] xl:h-[540px]`, object-cover; arrows + small dot indicators on the photo). Below the hero a clean centered intro block carries the eyebrow + lime tag pill, headline, subtitle and `HeroSearchBar` — text/search no longer sit on the image. Autoplay 4800ms, pause on hover/focus.
  - Mobile/tablet: compact app-like stack — headline + tag, then a swipeable `aspect-video` rounded photo card (dots overlay with generous hit areas), then a dedicated mobile search band (`HeroSearchSection`). Nothing squeeze-fit from desktop; hero is intentionally short.
- `PopularServicesSection` — জনপ্রিয় সেবা icon-card row right below the hero on ALL breakpoints: 6 cards — "বাসা ভাড়া, মেস, হোস্টেল" (no To-Let), "Electrician ও Plumber" (merged single card), কাজের বুয়া, বাসা পাল্টানো, গৃহশিক্ষক, রক্তদাতা — each = brand icon tile + Bengali label, linking to `/services/<slug>` aliases. Grid: 2-col mobile / 3 tablet / one 6-card desktop row. Hover lifts slightly; calm shadows.
- Service preview cards (`ServiceCard` + `ServiceCarousel`) are intentionally larger: image `aspect-[3/2]`, mobile rail `w-[80%]` (~1.2 cards visible), `sm:w-[52%]`, roomier body (`p-4/5`) and slightly larger type; all six sections use a uniform 4-col desktop grid.
- Navbar — solid sticky bar on EVERY page (no overlay, no measurement, no hero overlap by construction): white `h-16`/`sm:h-[4.5rem]` bar in normal flow, hero starts cleanly below it. Desktop: links + search + user chip + "সেবা নিন" CTA. Mobile: logo/word-mark left + hamburger right (44px targets, animated scrollable drawer, no 320px overflow); hero carries the search. Logo (`logo.png`) in navbar + footer.
- The former `ServiceCategoryBar` pills rail was removed; desktop shortcuts moved out of the hero into the dedicated `PopularServicesSection` icon-card row (all breakpoints).
- Six data-driven preview sections (বাসা ভাড়া, Electrician, Plumber, কাজের বুয়া, গৃহশিক্ষক, ব্লাড ডোনার) via `HomeServiceSection` + shared facades (mock fallback). Mappers in `lib/home-preview.ts`, loaders in `components/home/section-loaders.ts`. `ServiceCarousel`: horizontal snap rail on mobile (~1.4 cards) → grid on lg+; "আরও দেখুন" full-width on mobile. `ServiceCard`: flat `brand-700`/`brand-800` arrow box (rose for blood-donor tone), solid availability/verified chips (no blur), small image underlay gradient only.
- Compact sections (`py-9 sm:py-12`): কেন Mymensingh Sheba? stats strip flattened to a white bordered card, 3-step how it works, testimonials (mobile carousel), final CTA on flat deep-forest-green card (`bg-brand-800`, lime "সেবা খুঁজুন" + white outline "সেবাদাতা হিসেবে যুক্ত হন", no blobs/gradient).
- Dark-green `Footer` (2-col mobile / 12-col lg), fixed mobile bottom nav with safe-area padding (home root reserves `env(safe-area-inset-bottom)+5.5rem`).
