...
import { createStaffReport } from '@/lib/staff-service';
import type { StaffServiceKey } from '@/lib/staff-types';
import { STAFF_REPORT_REASONS } from '@/lib/staff-types';
import { cn } from '@/lib/utils';

interface StaffReportSheetProps {
  open: boolean;
  onClose: () => void;
  profileId: string;
...
export function StaffReportSheet({ open, onClose, profileId, serviceSlug, user }: StaffReportSheetProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSending(true);
    setError('');
    const result = await createStaffReport({
      profileId,
      serviceSlug,
      ...
      >
        <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between rounded-t-3xl">
          <div className="flex items-center gap-2 text-sm font-bold text-slate-900">
            ...
            <span>প্রোফাইল রিপোর্ট করুন</span>
          </div>
          ...
          {done ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <p className="text-xs text-slate-500 leading-relaxed">
                প্রোফাইলে কোনো সমস্যা থাকলে নিচের ফর্মটি পূরণ করুন। ভুয়া রিপোর্টেরও ব্যবস্থা নেওয়া হবে।
              </p>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-2">কারণ *</label>
                <div className="space-y-2">
                  {STAFF_REPORT_REASONS.map((r) => (
                    <button
                      key={r}
                      type="button"
                      onClick={() => {
              ...
              {!user && (
                <p className="text-[11px] text-slate-500 bg-slate-50 p-2.5 rounded-xl">
                  ...
                  <a href="/login" className="text-emerald-800 font-semibold underline">
                    লগইন
                  </a>{' '}
